<template>
  <el-row class="bar">
    ...
    >
      ...
      >
        <div style="display: inline-block; text-indent: 1em; width: 170px">
          {{ group.name }}
        </div>
        <div style="display: inline-block">
          <i
            :class="group.opened ? 'el-icon-arrow-down' : 'el-icon-arrow-right'"
          ></i>
        </div>
      </div>
        ...
        >
          ...
          >
            <div style="font-size: 20px; line-height: 40px">
              <svg-icon :className="chart" :iconClass="item.icon"></svg-icon>
    ...
    <div v-show="currentTab === 1">
      ...
      >
        <el-col :span="4" style="text-align: center"
          ><i :class="item.icon"
        /></el-col>
        <el-col :span="15" @click.native="showConfigBar($event, item, index)">{{
          item.cptTitle
        }}</el-col>
