    ...
    loadChart(option) {
      ...
      const convertData = function (toData) {
        ...
        for (let i = 0; i < toData.length; i++) {
          const toCoord = geoCoordMap[toData[i].name];
          if (toCoord) {
            res.push([startPosition.position,toCoord]);
          }
        }
        return res;
      };
      ...
      const seaData = function (toData){
        ...
        for (let i = 0; i < toData.length; i++) {
          let item = toData[i];
          let value = geoCoordMap[item.name].concat(item.value)
          res.push({name:item.name,value:value})
        }
        return res;
      }
      const seriesColor = '#a6c84c';
      let series = [];
      series.push({
            name: startPosition.name + ' Top10',
            type: 'lines',
            zlevel: 1,
            effect: {
              show: true,
